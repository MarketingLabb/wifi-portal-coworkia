...
# Escribir las reglas del anchor (solo se ejecuta una vez al inicio o al regenerar)
write_anchor_rules() {
    cat > "$PF_ANCHOR" << 'RULES'
# Tabla de IPs autenticadas — se actualiza sin recargar reglas
...
# Permitir internet a IPs autenticadas (tabla se actualiza atomicamente)
...
# Cargar reglas completas — solo al inicio o cuando cambia la estructura de reglas
# NO flushea sesiones activas cuando se actualiza la tabla
load_rules() {
    echo "🔄 Cargando reglas del firewall..."
    write_anchor_rules
    if pfctl -f /etc/pf.conf 2>/dev/null; then
        echo "✅ Reglas del firewall cargadas"
        return 0
    else
        echo "⚠️  Error cargando pf — revisa $PF_ANCHOR"
        return 1
    fi
}

# Sincronizar IPs desde DB — actualiza SOLO la tabla, sin recargar reglas
# Esto es atomico: no interrumpe conexiones TCP activas
sync_from_db() {
    ...
    echo "🔄 Actualizando tabla con $ip_count IP(s) activa(s)"

    # Reemplazar tabla atomicamente (SIN recargar reglas, SIN flushear estados)
    if [ -n "$ips" ]; then
    ...
    echo "✅ Tabla $TABLE_NAME actualizada: $ip_count IP(s)"
    return 0
}

# Agregar una IP directamente a la tabla (sin reload)
allow_ip() {
    local ip=$1
    if [ -z "$ip" ]; then
        echo "❌ Error: IP vacía"
    ...
    echo "✅ IP $ip agregada a tabla de autenticados"
}

# Remover una IP de la tabla (sin reload)
deny_ip() {
    local ip=$1
    if [ -z "$ip" ]; then
        echo "❌ Error: IP vacía"
    ...
    echo "✅ IP $ip removida de tabla de autenticados"
}

# Listar IPs autenticadas actuales
list_allowed() {
    echo "📋 IPs autenticadas en tabla $TABLE_NAME:"
    pfctl -a "$ANCHOR_NAME" -t "$TABLE_NAME" -T show 2>/dev/null || echo "  (tabla vacía o no cargada)"
}
...
cleanup_expired() {
    echo "🧹 Sincronizando sesiones expiradas..."
    sync_from_db
}
...
case "$1" in
    ...
    *)
        ...
        echo ""
        echo "Comandos:"
        echo "  sync-db [DB]  - Actualizar tabla de IPs desde DB (sin reload de reglas)"
        echo "  load          - Cargar reglas completas (solo al inicio)"
        echo "  regenerate    - Alias de load"
        echo "  allow IP      - Agregar IP a tabla de autenticados"
        echo "  deny IP       - Remover IP de tabla de autenticados"
        echo "  list          - Listar IPs autenticadas"
        echo "  cleanup       - Limpiar IPs de sesiones expiradas"
        exit 1
    local db_path="${1:-$DEFAULT_DB_PATH}"
